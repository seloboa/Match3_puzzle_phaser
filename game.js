...
class playGame extends Phaser.Scene {
    ...
    for (let i = 0; i < this.match3.getRows(); i++) {
      for (let j = 0; j < this.match3.getColumns(); j++) {
        let gemX =
          ...
          gameOptions.gemHeight * j +
          gameOptions.gemHeight / 2;
        let gemY =
          gameOptions.boardOffset.y +
          gameOptions.gemHeight * i +
          gameOptions.gemHeight / 2;
  ...
  gemSelect(pointer) {
    if (this.canPick) {
      this.dragging = true;
      let row = Math.floor(
        (pointer.y - gameOptions.boardOffset.y) / gameOptions.gemHeight
      );
      let col = Math.floor(
        (pointer.x - gameOptions.boardOffset.x) / gameOptions.gemHeight
      );
      ...
      function(movement) {
        this.tweens.add({
          ...
          x:
            ...
            gameOptions.gemHeight * movement.deltaColumn,
          y:
            this.match3.customDataOf(movement.row, movement.column).y +
            gameOptions.gemHeight * movement.deltaRow,
  ...
  makeGemsFall() {
    ...
    fallingMovements.forEach(
      function(movement) {
        ...
        this.tweens.add({
          ...
          y:
            this.match3.customDataOf(movement.row, movement.column).y +
            movement.deltaRow * gameOptions.gemHeight,
    ...
    replenishMovements.forEach(
      function(movement) {
        ...
        sprite.y =
          gameOptions.boardOffset.y +
          gameOptions.gemHeight * (movement.row - movement.deltaRow + 1) -
          gameOptions.gemHeight / 2;
        (sprite.x =
          gameOptions.boardOffset.x +
          gameOptions.gemHeight * movement.column +
          gameOptions.gemHeight / 2),
        ...
        this.tweens.add({
          ...
          y:
            gameOptions.boardOffset.y +
            gameOptions.gemHeight * movement.row +
            gameOptions.gemHeight / 2,
